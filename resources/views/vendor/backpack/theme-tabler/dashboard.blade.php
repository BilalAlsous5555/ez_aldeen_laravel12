...
@php
    ...
    foreach ($allStudents as $student) {
        ...
        if ($age < 8) $ageGroups['under8']++;
        elseif ($age <= 12) $ageGroups['bet8_12']++;
        elseif ($age <= 18) $ageGroups['bet12_18']++;
        else $ageGroups['over18']++;
    }
                ...
                <div class="card mb-0">
                    ...
                    <div class="table-responsive">
                        <table class="table table-vcenter card-table table-sm">
                            <thead><tr><th>اسم الطالب</th><th>الحضور</th><th>عذر الغياب</th><th>عدد الحضور هذا الشهر</th><th>عدد الغياب هذا الشهر</th><th>نسبة الحضور هذا الشهر</th></tr></thead>
                            <tbody>
                                @forelse($students as $student)
                                    @php
                                        $key = $student->id . '_' . $halqa->id;
                                        $sa = $attendanceData->get($key, collect());
                                        $todayRecord = $sa->first(fn($r) => $r->attendance_date->isSameDay(today()));
                                    ...
                                    @endphp
                                    <tr>
                                        <td><a href="{{ backpack_url('user/'.$student->id.'/show') }}">{{ $student->name }}</a></td>
                                        <td>{{ $todayRecord ? ($todayRecord->status === 'حاضر' ? 'حاضر' : 'غائب') : '—' }}</td>
                                        <td>{{ $todayRecord && $todayRecord->status === 'غائب' && $todayRecord->excused_reason ? $todayRecord->excused_reason : '—' }}</td>
                                        <td>{{ $monthlyPresent }}</td>
                                        <td>{{ $monthlyAbsent }}</td>
                                        <td>{{ $monthlyPct }}%</td>
                                    </tr>
                                @empty
                                    <tr><td colspan="6" class="text-muted text-center">لا يوجد طلاب</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
...
<div class="row">
    <div class="col-md-12">
        <div class="card mb-3">
            <div class="card-header"><h3 class="card-title"><i class="la la-trophy"></i> أكثر الطلاب التزاما في المعهد</h3></div>
            <div class="table-responsive">
                <table class="table table-vcenter card-table">
                    <thead><tr><th>اسم الطالب</th><th>عدد مرات الحضور الكلي</th><th>عدد مرات الغياب الكلي</th><th>النسبة المئوية الكلية</th></tr></thead>
                    <tbody>
                        @forelse($committedStudents as $cs)
                        <tr>
                            <td><a href="{{ backpack_url('user/'.$cs->student->id.'/show') }}">{{ $cs->student->name }}</a></td>
                            <td>{{ $cs->present }}</td>
                            <td>{{ $cs->absent }}</td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="progress progress-xs flex-grow-1">
                                        <div class="progress-bar {{ $cs->percentage >= 80 ? 'bg-success' : ($cs->percentage >= 60 ? 'bg-primary' : ($cs->percentage >= 40 ? 'bg-warning' : 'bg-danger')) }}" style="width: {{ $cs->percentage }}%"></div>
                                    </div>
                        ...
                        @empty
                        <tr><td colspan="4" class="text-muted text-center">لا توجد بيانات بعد</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<div class="row">
    <div class="col-md-6">
        <div class="card mb-3">
            <div class="card-header"><h3 class="card-title"><i class="la la-book"></i> آخر السور المحفوظة</h3></div>
            <table class="table table-vcenter card-table">
                <thead><tr><th>الطالب</th><th>السورة</th><th>تاريخ الإنجاز</th><th>المدرس</th></tr></thead>
                <tbody>
                    @forelse($recentSurahs as $a)
                    <tr>
                        <td><a href="{{ backpack_url('user/'.$a->student_id.'/show') }}">{{ $a->student->name }}</a></td>
                        <td>{{ $a->surah?->name ?? '—' }}</td>
    ...
    <div class="col-md-6">
        <div class="card mb-3">
            <div class="card-header"><h3 class="card-title"><i class="la la-layer-group"></i> آخر الأجزاء المحفوظة</h3></div>
            <table class="table table-vcenter card-table">
                <thead><tr><th>الطالب</th><th>الجزء</th><th>تاريخ الإنجاز</th><th>المدرس</th></tr></thead>
                <tbody>
